    ...
    def validate(self, data):
        ...
        if len(event_ids) > 1:  # Check only if more than one event
            ...
            for event_id in event_ids:
                event = Event.objects.get(id=event_id.id)  # Access the Event instance
    ...
    def get_events(self, obj):
        ...
        for event in events:
            if hasattr(event, "geoevent") and event.geoevent:
                serialized_events.append(GeoEventSerializer(event.geoevent).data)
            elif hasattr(event, "ringevent") and event.ringevent:
                serialized_events.append(RingEventSerializer(event.ringevent).data)
            elif hasattr(event, "boxevent") and event.boxevent:
                serialized_events.append(BoxEventSerializer(event.boxevent).data)
            else:
